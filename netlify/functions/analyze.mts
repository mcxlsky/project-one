...
// AI Gateway injects GEMINI_API_KEY automatically on Netlify, so no key config is needed.
const ai = new GoogleGenAI({})

// Auxiliary/functional words filtered out before deriving brand keywords.
...
export default async (req: Request, _context: Context) => {
  ...
  // Relax the stop-word filter if we got 0 keywords, to ensure some output.
  ...
  const candidatesList: { text: string; score: number; source: string }[] = []
  ...
  // Attempt to use Gemini (via Netlify AI Gateway) for name generation.
  try {
    let baseInstruction = ''
    if (baseName) {
...
Do not include any markdown formatting (like \`\`\`json) or extra text. Return just the raw JSON array.`
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      parsed.forEach((item: any) => {
        if (item.text) addCandidate(item.text, item.score || 90, item.source || 'AI Synthesized')
      })
    }
  } catch (err: any) {
  ...
  // Always mix in algorithmic logic to ensure favorites like Vowel Drop appear.
