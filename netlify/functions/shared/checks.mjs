// Shared helpers for the brand-checker API, ported from the original Express
// server (server.js) so the same logic runs inside Netlify Functions.
...
export async function checkSocialAvailability(platform, handle) {
...
// Algorithmic name suggestions (legacy fallback endpoint)
export function generateSuggestions(name, category) {
...
// Generate brand-name candidates: Gemini AI when a key is present, always
// blended with algorithmic candidates. `apiKey` is read from the environment
// by the caller so this stays portable.
export async function analyzeNames(description, baseName, apiKey) {
  ...
  // Attempt to use Gemini AI for name generation when a key is configured
  try {
    if (apiKey) {
...
Do not include any markdown formatting (like \`\`\`json) or extra text. Return just the raw JSON array.`;
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        parsed.forEach(item => {
          if (item.text) {
            addCandidate(item.text, item.score || 90, item.source || 'AI Synthesized');
          }
        });
      }
    }
